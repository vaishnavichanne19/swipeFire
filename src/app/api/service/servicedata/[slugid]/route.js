...
export async function GET(req, { params }) {
  try {
    ...
    const SingleData = await ServiceData.findById(slugid);
...
export async function PUT(req, { params }) {
  try {
    ...
    const imageFields = ["serviceimage"];
    ...
    for (const field of imageFields) {
      ...
      if (file && file.size > 0) {
        ...
        const uploadResult = await new Promise((resolve, reject) => {
          cloudinary.uploader
            .upload_stream({ folder: "Service Img" }, (error, result) => {
    ...
    const updatedData = await ServiceData.findByIdAndUpdate(
      slugid,
      { $set: updatedPayload },
      { new: true },
    );
