...
export async function GET(req, { params }) {
  try {
    ...
    const getdata = await ProductionData.findById(slugid);
...
export async function PUT(req, { params }) {
  try {
    ...
    const existid = await ProductionData.findById(slugid);
...
export async function DELETE(req, { params }) {
  ...
  try {
    ...
    const existdata = await ProductionData.findById(slugid);

    if (!existdata) {
      return NextResponse.json({ message: "User Not Found" });
    }

    await ProductionData.findByIdAndDelete(slugid);
    return NextResponse.json(
