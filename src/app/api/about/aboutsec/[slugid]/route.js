...
import { AboutData } from "../../../../../../modules/AboutPageModule/AboutSec";
...
export async function GET(req, { params }) {
  try {
    ...
    const SingleData = await AboutData.findById(slugid);
...
if (
  ...
  updatedPayload.description = description;
    ...
    for (const field of imageFields) {
      ...
      if (file && file.size > 0) {
        ...
        const uploadResult = await new Promise((resolve, reject) => {
          cloudinary.uploader
            .upload_stream({ folder: "About Img" }, (error, result) => {
    ...
    const updatedData = await AboutData.findByIdAndUpdate(
      slugid,
      { $set: updatedPayload },
      { new: true },
    );
