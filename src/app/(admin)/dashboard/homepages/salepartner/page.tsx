...
export default function UserInfoCard() {
  ...
  useEffect(() => {
    ...
    const fetchSingleData = async () => {
      try {
        const res = await axios.get(`/api/home/salepartner/${editId}`);
  ...
  if (selectedImages[key]) {
    formData.append(key, selectedImages[key] as File);
  }
});

    try {
      const res = await axios.put(`/api/home/salepartner/${editId}`, formData);
      if (res.data.success) {
                  ...
                  {UpdateData.productimg && (
                    <div className="grid grid-cols-1 gap-4 ">
                      <div>
                        ...
                        <Input
                          type="file"
                          onChange={(e) => handleImageChange(e, "productimg")}
                          className="border w-full p-2 my-2"
                        />

                        <Image
                          src={UpdateData.productimg}
                          alt="productimg"
                          width={200}
                          height={120}
