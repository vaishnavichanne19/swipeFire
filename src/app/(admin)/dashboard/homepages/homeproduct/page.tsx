...
export default function UserInfoCard() {
  const [fetchProductData, setFetchProductData] = useState<HeroData[]>([]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("/api/product/productdata");
        setFetchProductData(response.data.data);
      } catch (error) {
        console.log(error);
      }
    };
  ...
  return (
    <div>
      <div>
        ...
        {fetchProductData.slice(0, 1).map((data) => (
          <div
            key={data._id}
          ...
          >
            <div className="flex flex-col-reverse gap-6  lg:flex-row lg:items-start lg:justify-between">
              ...
              <div className="flex justify-end">
                <Link
                  href={`/dashboard/product/productdata/${data._id}?from=homeproduct`}
      ...
      <div className="flex justify-end mr-4 my-5">
        <Link
          href="/dashboard/product/productlist/addpage?from=homeproduct"
                    ...
                    <TableRow key={data._id}>
                      ...
                      <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                        <div className="flex items-center flex-wrap gap-2">
                          ...
                          <Link
                            href={`/dashboard/product/productlist/${data._id}?from=homeproduct`}
