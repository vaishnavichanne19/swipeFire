...
export default function UserInfoCard() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get("/api/about/corevalue");
  ...
  useEffect(() => {
    ...
    const fetchSingleData = async () => {
      try {
        const res = await axios.get(`/api/about/corevalue/${editId}`);
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      await axios.put(`/api/about/corevalue/${editId}`, UpdateData);
  ...
  const Deletedata = async (UserId: string) => {
    try {
      await axios.delete(`/api/about/corevalue/${UserId}`);
      setFetchAllData((prev) => prev.filter((user) => user._id !== UserId));
      toast.error("Data Deleted Successfully!");
      router.push("/dashboard/about/corevalue");
    } catch (error) {
