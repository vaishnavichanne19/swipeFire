...
const Update = () => {
  ...
  useEffect(() => {
    const fetchhomeproduct = async () => {
      const res = await axios.get(`/api/blog/blogdata/${editid}`);
      if (res.data.success) {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      await axios.put(`/api/blog/blogdata/${editid}`, formData);
      toast.info("Data Updated Successfully");
      router.push("/dashboard/blog/blogdata");
    } catch (error) {
