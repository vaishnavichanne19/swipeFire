...
const Update = () => {
  ...
  useEffect(() => {
    const fetchhomeproduct = async () => {
      const res = await axios.get(`/api/service/servicedata/${editid}`);
      if (res.data.success) {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      await axios.put(`/api/service/servicedata/${editid}`, formData);
      toast.info("Data Updated Successfully");
      router.push("/dashboard/service/servicedata");
    } catch (error) {
        ...
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          ...
          <form onSubmit={handleUpdate} className="flex flex-col ">
            ...
            <div className="flex items-center gap-3 px-2 mt-6 justify-end">
              <Link
                href="/dashboard/service/servicedata"
