...
export default function UserInfoCard() {
  ...
  useEffect(() => {
    ...
    const fetchSingleData = async () => {
      try {
        const res = await axios.get(`/api/service/ourpriority/${editId}`);
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      await axios.put(`/api/service/ourpriority/${editId}`, UpdateData);
  ...
  const Deletedata = async (UserId: string) => {
    try {
      await axios.delete(`/api/service/ourpriority/${UserId}`);
      setFetchAllData((prev) => prev.filter((user) => user._id !== UserId));
      toast.error("Data Deleted Successfully!");
      router.push("/dashboard/service/ourpriority");
    } catch (error) {
              ...
              <div className="flex justify-end">
                <div className="relative inline-block ">
                  ...
                  <Dropdown
                    ...
                    className="w-40 p-2"
                  >
                    <Link href={`ourpriority/${data._id}`}>
                      <DropdownItem
                        tag="a"
