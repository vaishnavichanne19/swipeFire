...
export default function ProductSell() {
  ...
  const DeleteProductTypedata = async (prodId: string) => {
    try {
      await axios.delete(`/api/product/producttypelist/${prodId}`);

      setFetchProductTypeData((prev) =>
        prev.map((item) => ({
          ...item,
          prodtype: item.prodtype.filter((p) => p._id !== prodId),
        })),
      );
  ...
  const allProducts = fetchProductTypeData.flatMap((item) => item.prodtype);
                      ...
                      <TableRow key={data._id}>
                        ...
                        <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() => DeleteProductTypedata(data._id)}
