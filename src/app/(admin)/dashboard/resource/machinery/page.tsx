...
export default function UserInfoCard() {
  ...
  useEffect(() => {
    ...
    const fetchSingleData = async () => {
      try {
        const res = await axios.get(`/api/resource/machinery/${editId}`);
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      await axios.put(`/api/resource/machinery/${editId}`, UpdateData);
  ...
  const Deletedata = async (UserId: string) => {
    try {
      await axios.delete(`/api/resource/machinery/${UserId}`);
      setFetchAllData((prev) => prev.filter((user) => user._id !== UserId));
      toast.error("Data Deleted Successfully!");
      router.push("/dashboard/resource/machinery");
    } catch (error) {
